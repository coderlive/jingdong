<%@ page language="java" import="java.util.*" import="vo.Properties,page.PageInfo"
...
<div id="categoryDiv">
    <!--0.8 搜索功能 加入搜索框-->
    <form id="categoryForm2" action="AttributeServlet?action=getPageByQuery" method="post">
    ...
    <div class="text-center categoryTableDiv">
        ...
        <table id="categoryTable" class="table table-striped table-bordered table-hover">
            ...
            <c:forEach items="${list}" var="properties">
                <tr>
                    <td>${properties.prp_id}</td>
                    <td>${properties.cid}</td>
                    <td>${properties.pname}</td>

                    <td>
                        <button onclick="getOneForUpdate(${properties.prp_id})" class="btn btn-success btn-xs">修改</button>
                    </td>
                    <td>
                        <button onclick="deletePropertiesById(${properties.prp_id})" class="btn btn-danger btn-xs">删除</button>
                    </td>
    ...
    <div class="pageDiv">
        ...
        <form id="categoryForm" action="AttributeServlet" method="post">
            ...
            <a id="first" class="btn btn-default btn-sm" href="AttributeServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>


            <a id="previous" class="btn btn-default btn-sm"
               href="AttributeServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>"> 上一页</a>
            <a id="next" class="btn btn-default btn-sm"
               href="AttributeServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>"> 下一页</a>


            <a id="last" class="btn btn-default btn-sm"
               href="AttributeServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
...
<script type="text/javascript">
    ...
    function deletePropertiesById(id) {
        $.confirm({
            title: '',
            content: '您确认删除吗?',
            confirm: function () {

                location.assign("AttributeServlet?action=delete&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&prp_id=" + id);
            },
            cancel: function () {
    ...
    function getOneForUpdate(id) {
        ...
        location.assign("AttributeServlet?action=getOneForUpdate&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&prp_id=" + id);
    }
