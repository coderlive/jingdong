<%@ page language="java" import="java.util.*,vo.*,page.PageInfo,service.impl.CategoryServiceImpl"
         pageEncoding="UTF-8" %>
<%
        ...
        <table id="productTable" class="table table-striped table-bordered table-hover">
            ...
            <tr>
                ...
                <%
                    ...
                    Categorys category = service.selectById(cid);
                %>
                <td><%=category.getCname() %>
                </td>
                <td>
    ...
    <div class="pageDiv">
        <form id="productForm2" action="ProductServlet?action=getPageByQuery&target=productMainUpload" method="post">
            <a id="first" class="btn btn-default btn-sm"
               href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>


            <a id="previous" class="btn btn-default btn-sm"
               href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'>
                上一页</a>
            <a id="next" class="btn btn-default btn-sm"
               href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'>
                下一页</a>


            <a id="last" class="btn btn-default btn-sm"
               href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>'>尾页</a>
...
<script type="text/javascript">
    ...
    function getOneForUpload(pid){
        location.assign("ProductServlet?action=getOneForUpload&target=uploadProductImage&searchCondition=${searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
    }
</script>
<%
