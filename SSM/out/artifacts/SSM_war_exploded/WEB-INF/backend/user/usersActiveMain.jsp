<%@ page language="java"
         import="java.util.*,com.vo.*,com.page.PageInfo,com.util.*"
         pageEncoding="UTF-8" %>
<%
...
<div id="categoryDiv">
    <form id="categoryForm" action="user/getPageByQuery" method="get">
        <div id="jdCategorySearch" class="input-group">
            <input name="target" type="hidden" value="/backend/user/usersActiveMain">
            <input name="requestPage" type="hidden" value="${requestPage}">
    ...
    <div class="text-center categoryTableDiv">
            ...
            <tr>
                ...
                <td>
                    <%
                        ...
                        if (active == 1) {
                    %>
                    <button onclick="usersActive(<%=users.getUserid()%>)" class="btn btn-success btn-xs disabled"
                            disabled="disabled">激活
                    </button>
                    <%
                    } else {
                    %>
                    <button onclick="usersActive(<%=users.getUserid()%>)" class="btn btn-success btn-xs">激活</button>
                    <%
                        }
                    %>
                ...
                <td>
                    <%
                        if (active == 0) {
                    %>
                    <button onclick="usersUnactive(<%=users.getUserid()%>)" class="btn btn-danger btn-xs disabled"
                            disabled="disabled">取消激活
                    </button>
                    <%
                    } else {
                    %>
                    <button onclick="usersUnactive(<%=users.getUserid()%>)" class="btn btn-danger btn-xs">取消激活</button>
                    <%
                        }
                    %>
        ...
        <div class="pageDiv">
            <form id="productForm2" action="user/getPageByQuery?target=/backend/user/usersActiveMain"
                  method="post">
                <a id="first" class="btn btn-default btn-sm"
                   href='user/getPageByQuery?target=/backend/user/usersActiveMain&searchCondition=${searchCondition}&requestPage=1'>首页</a>


                <a id="previous" class="btn btn-default btn-sm"
                   href='user/getPageByQuery?target=/backend/user/usersActiveMain&searchCondition=${searchCondition}&requestPage=${pageInfo.previousPage}'>
                    上一页</a>
                <a id="next" class="btn btn-default btn-sm"
                   href='user/getPageByQuery?target=/backend/user/usersActiveMain&searchCondition=${searchCondition}&requestPage=${pageInfo.nextPage}'>
                    下一页</a>


                <a id="last" class="btn btn-default btn-sm"
                   href='user/getPageByQuery?target=/backend/user/usersActiveMain&searchCondition=${searchCondition}&requestPage=${pageInfo.totalPageCount}'>尾页</a>
   ...
   <span>共${pageInfo.totalRecordCount}条记录 </span>
    ...
    <script type="text/javascript">
        ...
        function usersUnactive(userid) {
            $.confirm({
                title: ' ',
                content: '您确认把编号为' + userid + '的用户取消激活吗?',
                confirm: function () {
                    //发请求
                    location.assign("user/updateActive?target=/backend/user/usersActiveMain&active=0&searchCondition=${searchCondition}&requestPage=${pageInfo.getCurrentPage()}&userid=" + userid);
                },
                cancel: function () {
        ...
        function usersActive(userid) {
            $.confirm({
                title: ' ',
                content: '您确认激活编号为' + userid + '的用户吗?',
                confirm: function () {
                    //发请求
                    location.assign("user/updateActive?target=/backend/user/usersActiveMain&active=1&searchCondition=${searchCondition}&requestPage=${pageInfo.getCurrentPage()}&userid=" + userid);
                },
                cancel: function () {
