...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public List salesVolume(Integer row) {
        List<Products> list= pd.salesVolume(row);
        return list;
    }

    @Override
    public int getPageQueryByCount(Products p) {
        System.out.println("要根据条件查询的东西"+p);
        return pd.getPageQueryByCount(p);
    }

    @Override
    public List<Products> getPageQuery(PageInfo pageInfo, Products p, OrderCondition oc) {
        ...
        return pd.getPageQuery(sql);
    }

    @Override
    public Products selectById(Integer pid) {
        String sql="select * from products where pid="+pid;
        return pd.selectBySql(sql);
    }

    @Override
    public Products selectOne(Products p) {
    ...
    public void updateProduct(Products p) {
        pd.updateByPrimaryKeySelective(p);
    }
    ...
    @Override
    public void deleteById(Products p) {
        pd.deleteByPrimaryKey(p);
    }
    ...
    public void saveImagePathInTable(String pid,List<String> fileNameWithDirectoryList) throws Exception{
        ...
        String sql="select * from products where pid="+pid;
        try {
            product = pd.selectBySql(sql);//先查出来再覆盖
        } catch (Exception e1) {
            e1.printStackTrace();
