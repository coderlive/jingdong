...
<div id="categoryDiv">
    <!--0.8 搜索功能 加入搜索框-->
    <form id="categoryForm2" action="attribute/getAttribute" method="post">
    ...
    <!--0.8 上一页 下一页时 都要请求getAttribute方法 并把查询条件传过去-->
    <div class="pageDiv">

        <form id="categoryForm" action="attribute/getAttribute" method="post">
            <input type="hidden" name="searchCondition" value='${requestScope.searchCondition}'/>
            <a id="first" class="btn btn-default btn-sm" href="attribute/getAttribute?searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>


            <a id="previous" class="btn btn-default btn-sm"
               href="attribute/getAttribute?searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>"> 上一页</a>
            <a id="next" class="btn btn-default btn-sm"
               href="attribute/getAttribute?searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>"> 下一页</a>


            <a id="last" class="btn btn-default btn-sm"
               href="attribute/getAttribute?searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
...
<script type="text/javascript">
    ...
    function deletePropertiesById(id) {
        $.confirm({
            ...
            confirm: function () {
                ...
                location.assign("attribute/delete?searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&prp_id=" + id);
            },
            cancel: function () {
    ...
    function getOneForUpdate(id) {
        ...
        location.assign("attribute/getOneForUpdate?searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&prp_id=" + id);
    }
